...
export class IntentionHandler {
  ...
  static async handleDataProcess(
    ...
    const prompt = `
      ...
      主要有如下一些情况：
      (1). 根据用户要求和附带的数据，进行数据的洞察/剖析/透视/阐释/推演/解构/溯源/思辨/融合；如果能够直接给出处理结果，则输出Markdown形式的分析结果。优先以这种情况进行处理。
  ...
  static getMyIntentionExamples(userId: UUID) {
    const intentionNote = [
      ...
      '2. 剖析这些爆文的标题是怎么写的',
      '3. 解构前5个爆文',
      '4. 找出点赞次数最多的5篇，对比剖析其互动率',
      '5. 找出转发次数最多的5篇，对比阐释其互动率',
      '6. 找出收藏次数最多的5篇，对比分析其互动率',
      '7. 推演预测下周可能会火的内容方向',
