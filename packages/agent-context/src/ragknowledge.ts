...
export class RAGKnowledgeManager implements IRAGKnowledgeManager {
    ...
    private hasProximityMatch(text: string, terms: string[]): boolean {
        ...
        for (let i = 0; i < allPositions.length - 1; i++) {
            if (Math.abs(allPositions[i] - allPositions[i + 1]) <= 5) {
                data3Logger.debug("[Proximity Match]", {
                    terms,
    ...
    async createKnowledge(item: RAGKnowledgeItem): Promise<void> {
        if (!item.content.text) {
            data3Logger.warn("Empty content in knowledge item");
            return;
        }
        ...
        try {
            ...
            data3Logger.error(`Error processing knowledge ${item.id}:`, error);
            throw error;
        }
    }
    ...
    async cleanupDeletedKnowledgeFiles() {
        try {
            ...
            data3Logger.debug("[Cleanup] Finished knowledge cleanup process");
        } catch (error) {
            data3Logger.error(
    ...
    async processFile(file: {
        ...
        const timeMarker = (label: string) => {
            ...
            data3Logger.info(`[Timing] ${label}: ${time.toFixed(2)}s`);
        };
        ...
        try {
            ...
            data3Logger.info(`Generated ${totalChunks} chunks`);
            ...
            data3Logger.error(`Error processing file ${file.path}:`, error);
            throw error;
        }
    }
}
