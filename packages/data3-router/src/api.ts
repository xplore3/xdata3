...
// import type { TeeLogQuery, TeeLogService } from "@data3os/plugin-tee-log";
// import { REST, Routes } from "discord.js";
import type { DirectClient } from ".";
import { validateUuid } from "@data3os/agentcontext";
...
export function createApiRouter(
    ...
    router.post("/agents/:agentId/set", async (req, res) => {
        ...
        try {
            ...
            data3Logger.error(`Error parsing character: ${e}`);
            res.status(400).json({
                success: false,
                message: e.message,
            });
        ...
        try {
            ...
            data3Logger.log(`${character.name} started`);
        } catch (e) {
            data3Logger.error(`Error starting agent: ${e}`);
            res.status(500).json({
                success: false,
                message: e.message,
            });
    ...
    //     } catch (error) {
    //         data3Logger.error("Failed to get TEE agents:", error);
    //         res.status(500).json({
    ...
    //     } catch (error) {
    //         data3Logger.error("Failed to get TEE agent:", error);
    //         res.status(500).json({
    ...
    //             });
    //         } catch (error) {
    //             data3Logger.error("Failed to get TEE logs:", error);
    //             res.status(500).json({
