...
class Server {
    ...
    private setupRoutes() {
        this.app.get("/health", async (req, res) => {
            ...
            const item5_value = await this.getHealthResponce(item5_name);

            const item6_name = "hot_kols";
            const item6_value = await this.getHealthResponce(item6_name);

            res.status(200).json({ [item1_name]: item1_value, [item2_name]: item2_value, [item3_name]: item3_value, [item4_name]: item4_value, [item5_name]: item5_value, [item6_name]: item6_value });
        });
